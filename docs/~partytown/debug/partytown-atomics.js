...
((window) => {
    ...
    if (!added.has(obj)) {
      ...
      for (propName in obj) {
        if (isValidMemberName(propName)) {
          propValue =
            'path' === propName && getConstructorName(obj).endsWith('Event') ? obj.composedPath() : obj[propName];
      ...
      try {
        ...
        if (1 === applyPath[0] && applyPath[1] in winCtxs[winId].$window$) {
          ...
          if (instance) {
            ...
            isLast &&
              (accessRsp.$rtnValue$ = serializeForWorker(winId, rtnValue, void 0, void 0, void 0, task.$instanceId$));
          } else {
  ...
  })((accessReq, responseCallback) => mainAccessHandler(worker, accessReq).then(responseCallback)).then(
    (onMessageHandler) => {
      if (onMessageHandler) {
